...
# ─────────────────────────────────────────────
# Debug helpers (single/retry/repair 공통)
# ─────────────────────────────────────────────
...
def run_planner_coder(config_path: str):
    ...
    output_dir = output_cfg.get("dir", f"results/RUN/{dataset_name}/planner_coder")
    ...
    try:
        ...
        for i in range(samples_to_run):
            ...
            # sample 모드일 때 단계별 실행 흐름 출력 (코드 추출 전)
            if debug_mode == "sample":
                _print_sample_execution_flow(
                    sample=sample,
                    prompt=coder_prompt,
                    raw_text=coder_raw_text,
                )

            # 4-1-check. 모델 출력이 빈 값인지 확인
            if coder_output_tokens == 0 or not coder_raw_text.strip():
