# src/orchestration/adaptive_policy.py
"""
Adaptive Policy Orchestration
==============================

목적:
  현재 상태(PolicyState)를 보고 generate / retry / repair / plan / stop 중
  하나를 선택해서 orchestration을 진행하는 상위 정책 모듈.
  ...
  4단계  비교 실험 준비       : method="adaptive_policy"로 저장, 기존 결과와 동일한
                                포맷 유지 → 외부 분석 스크립트가 바로 사용 가능
  ...
  Action 정의:
    ...
    plan      – planner를 호출해 구현 계획을 먼저 생성
